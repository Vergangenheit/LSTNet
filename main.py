...
from utils import *
from train_eval import train, evaluate, makeOptimizer, Optimizer
...
# Select model
model: Module = eval(args.model).Model(args, Data)
train_method = train
eval_method = evaluate
nParams: int = sum([p.nelement() for p in model.parameters()])
...
optim: Optimizer = makeOptimizer(model.parameters(), args)
...
try:
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        train_loss: float = train_method(Data, Data.train[0], Data.train[1], model, criterion, optim, args)

        val_loss, val_rae, val_corr = eval_method(Data, Data.valid[0], Data.valid[1], model, evaluateL2, evaluateL1,
                                                  args)
        print(
        ...
        if val_loss < best_val:
            print(colorama.Fore.YELLOW + "Saving model")
